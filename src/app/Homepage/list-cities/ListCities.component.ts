...
import {
  ...
  Tag,
  Photo,
} from 'src/app/service/CityService.service';
...
export class ListCitiesComponent implements OnInit {
  ...
  constructor(private cityService: CityService) {
    combineLatest([
      cityService.cityDataSource$,
      cityService.tagDataSource$,
      cityService.regionDataSource$,
      cityService.photoDataSource$,
      cityService.tagCacheDataSource$,
      cityService.regionCacheDataSource$,
      cityService.photoCacheDataSource$,
    ]).subscribe(
      ([cities, tags, regions, photos, tagCache, regionCache, photoCache]) => {
        if (
          !cities ||
          !tags ||
          !regions ||
        ...
        this.regions.forEach((region) => {
          ...
          this.regionIdToCities.set(
            region.regionId,
            this.allCities.filter((city) => city.regionId === region.regionId)
          );
        });
  ...
  onWindowResize(event: any) {
    if (Number(event.target.innerWidth) > 800) {
      this.itemsPerRow = 4;
    } else {
      this.itemsPerRow = 2;
    }
  ...
  getDummyArrayOfRowNeeded(): number[] {
    ...
    const length = Math.max(
      1,
      this.filteredCities.length / this.itemsPerRow +
        Number(this.filteredCities.length % this.itemsPerRow !== 0)
    );
  ...
  onCollapseAllButtonClicked(target: HTMLElement): void {
    this.showedRow = Math.min(
      this.displayedCities.length,
      this.minRowDisplayed
    );
  ...
  private filterCityOnTagsChanged(): void {
    ...
    this.filteredCities = this.allCities.filter((city) => {
      return (
        city.tags.filter((tag) => this.selectedTagIds.has(tag.tagId)).length ===
  ...
  private addTagPhotoRegionToCities(): void {
    this.allCities.forEach((city) => {
      city.tags = city.tagIds
        .map((tagId) => this.tagCache.get(tagId))
        .filter((tag) => !!tag);
      city.photos = city.photoIds.map((photoId) =>
        this.photoCache.get(photoId)
      );
